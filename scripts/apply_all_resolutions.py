...
Fully autonomous: derives everything from the decompiled code + decoded string map.

Resolution sources (all computed at runtime, no pre-captured data needed):
1. Decoded string map (generated by v8dasm --decode-strings)
2. Closure chain formulas (1,800+ computed by trace_closures engine from decompiled.js)
3. Ghidra-extracted formulas (optional, from headless analysis)

Output: split per-category JS files + comprehensive report.

Usage:
  python3 apply_all_resolutions.py \\
    --input decompiled.js --index index.json \\
    --string-map string_map.json --output-dir ./resolved \\
    [--ghidra-formulas wrapper_formulas.json] [--ghidra-context context_resolutions.json]
"""
...
def main():
    ...
    parser.add_argument("--string-map", type=Path, required=True,
                        help="Path to decoded string map JSON (from v8dasm --decode-strings)")
    parser.add_argument("--ghidra-formulas", type=Path, default=None,
                        help="Optional: Ghidra-extracted wrapper formulas JSON")
    parser.add_argument("--ghidra-context", type=Path, default=None,
                        help="Optional: Ghidra context-dependent resolutions JSON")
    ...
    string_map = tc.load_decoder_log(str(args.string_map))
    log(f"  Decoded strings: {len(string_map)}")
    ...
    # Load optional Ghidra formulas (boost, not required)
    v8_formulas = {}
    if args.ghidra_formulas and args.ghidra_formulas.exists():
        v8_formulas = tc.load_v8_wrapper_formulas(str(args.ghidra_formulas))
        log(f"  Ghidra wrapper formulas: {len(v8_formulas)}")
    else:
        log("  Ghidra wrapper formulas: none (trace_closures will compute all)")

    ghidra_formulas, bc_formulas, decomp_to_bc = {}, {}, {}
    if args.ghidra_context and args.ghidra_context.exists():
        ghidra_formulas, bc_formulas, decomp_to_bc = tc.load_ghidra_resolutions(
            str(args.ghidra_context)
        )
        log(f"  Ghidra context formulas: {len(ghidra_formulas)} decomp + {len(bc_formulas)} bc")
    else:
        log("  Ghidra context formulas: none (trace_closures will compute all)")
    ...
    report = {
        ...
        "pipeline": "autonomous (no pre-captured data)",
        "summary": {
        ...
        "data_sources": {
            ...
            "ghidra_seed_formulas": len(v8_formulas) + len(ghidra_formulas),
            "trace_closures_formulas": len(all_formulas),
        },
